...
表达式中的数字系数是当前全站货币 / 1M tokens 的价格。例如 p * 2.5 表示输入价格为 2.50 个当前全站货币单位 / 1M tokens。
...
export default function TieredPricingEditor({
  ...
  const evalResult = useMemo(() => {
    ...
    if (!result.error) {
      result.cost = (result.cost / 1000000) * getSiteCreditsPerPriceUnit();
    }
    return result;
  }, [
    effectiveExpr,
