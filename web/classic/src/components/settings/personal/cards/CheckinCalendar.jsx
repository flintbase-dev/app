...
import HCaptchaWidget from '../../../common/HCaptchaWidget';
import { API, showError, showSuccess, renderQuota } from '../../../../helpers';

const CheckinCalendar = ({ t, status, hCaptchaEnabled, hCaptchaSiteKey }) => {
  const [loading, setLoading] = useState(false);
  const [checkinLoading, setCheckinLoading] = useState(false);
  const [hCaptchaModalVisible, setHCaptchaModalVisible] = useState(false);
  const [hCaptchaWidgetKey, setHCaptchaWidgetKey] = useState(0);
  ...
  const postCheckin = async (token) => {
    return API.mutation('checkin', {
      params: token ? { hcaptcha: token } : {},
    });
  };

  const shouldTriggerHCaptcha = (message) => {
    if (!hCaptchaEnabled) return false;
    if (typeof message !== 'string') return true;
    return message.includes('hCaptcha');
  };
  ...
  const doCheckin = async (token) => {
    ...
    try {
      ...
      if (success) {
        ...
        setHCaptchaModalVisible(false);
      } else {
        if (!token && shouldTriggerHCaptcha(message)) {
          if (!hCaptchaSiteKey) {
            showError('hCaptcha is enabled but site key is empty.');
            return;
          }
          setHCaptchaModalVisible(true);
          return;
        }
        if (token && shouldTriggerHCaptcha(message)) {
          setHCaptchaWidgetKey((v) => v + 1);
        }
